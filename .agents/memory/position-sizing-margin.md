...
**How to apply:** `RiskManager.calculate_position_size` in `ai_trading_advisor.py` takes `min(risk_based_lots, margin_based_lots)`, where margin_based_lots is derived from `(balance * MARGIN_SAFETY_FACTOR - used_margin) / margin_per_lot`. `CTRADER_LEVERAGE` env var (default 100) is now only the last-resort fallback.

**Superseded again — query real margin from the server, don't assume a flat leverage:** not every broker/account has the same leverage, and cTrader's `ProtoOAExpectedMarginReq`/`_RES` (payloadType 2139/2140) returns the real per-volume buy/sell margin requirement directly (already reflecting that account's true leverage/tiered rules), taking `symbolId` + `volume` (same lot✕10,000,000 scale as order placement) and returning `moneyDigits`-scaled `buyMargin`/`sellMargin`. `mcp_server.get_expected_margin(symbol, [1.0])` wraps this with a try/except returning `None` if unsupported, so callers cleanly fall back to the `CTRADER_LEVERAGE` estimate — confirmed by comparing against a real open position's `usedMargin` field (exact match). Note: margin per "1.0" internal lot unit varies hugely by symbol (e.g. XAUUSD legitimately needed ~$820k for what the codebase calls "1.0 lot" on this account, due to `lotSize` differing from the forex-standard 100,000 units) — trust the server number even if it looks surprising, don't re-derive it from `contract_size` assumptions.
